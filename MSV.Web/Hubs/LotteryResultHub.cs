...
namespace ADSoft.Web.Hubs
{
    public class LotteryResultHub : Hub
    {
        ...
        public void SendDiscussMessage(string username, string avatar, string message, DateTime createdDate)
        {
            var context = GlobalHost.ConnectionManager.GetHubContext<LotteryResultHub>();
            context.Clients.All.ClientUpdateDiscussMessage(username, avatar, message, createdDate);
        }
        ...
        public void SendLotteryResult(string name, string message)
        {
            ...
            Lottery lottery = LotteryRepository.FindOneBy(l => l.JackpotsDate.Day == DateTime.Now.Day);
            if (lottery != null)
            {
                lottery.Html = content;
                LotteryRepository.UpdateAsync(lottery);
            }
        ...
        private void parseLottery(string tableContent)
        {
            ...
            XElement table = XElement.Parse(tableContent.Replace("&nbsp;", ""));
